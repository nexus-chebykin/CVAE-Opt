# ------------------------------------------------------------------------------+
# Optuna-based Hyperparameter Tuning for EvoX SHADE Optimizer
#
# This script performs automated hyperparameter search for the SHADE algorithm
...
def setup_logging(output_dir: str) -> Tuple[str, logging.Logger]:
    ...
    logger = logging.getLogger('optuna_shade_tuning')
...
def create_config_for_trial(base_config, diff_padding_num: int):
    """Create a configuration object for a trial with specific SHADE parameters."""
    class Config:
        pass
...
def objective(trial: optuna.Trial, args) -> float:
    ...
    logger.info(f"  Parameters: diff_padding_num={diff_padding_num}")

    # Create config for this trial
    config = create_config_for_trial(base_config, diff_padding_num)
...
def main():
    parser = argparse.ArgumentParser(
        description="Optuna-based hyperparameter tuning for EvoX SHADE optimizer"
    )
    ...
    parser.add_argument('--study_name', type=str, default='shade_tuning',
                       help='Optuna study name (default: shade_tuning)')
    parser.add_argument('--storage', type=str, default='sqlite:///optuna_shade.db',
                       help='Optuna storage (default: sqlite:///optuna_shade.db)')
    ...
    logger.info("=" * 80)
    logger.info("OPTUNA HYPERPARAMETER TUNING FOR EVOX SHADE")
    logger.info("=" * 80)
    ...
    best_params_file = os.path.join(output_dir, 'best_shade_params.json')
    best_params_data = {
