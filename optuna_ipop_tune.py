...
def setup_logging(output_dir: str) -> Tuple[str, logging.Logger]:
    ...
    # Create logger
    logger = logging.getLogger('optuna_ipop_tuning')
...
def create_config_for_trial(base_config, sigma0: float, CMA_rankmu: float, CMA_rankone: float):
    """
    Create a configuration object for a trial with specific IPOP-CMA-ES parameters.
    ...
    Returns:
        Config object with IPOP-CMA-ES parameters set
    """
    ...
    # Set IPOP-CMA-ES-specific parameters (these are what we're tuning)
    config.ipop_sigma0 = sigma0
...
def main():
    parser = argparse.ArgumentParser(
        description="Optuna-based hyperparameter tuning for IPOP-CMA-ES optimizer"
    )
    ...
    parser.add_argument('--batch_size', type=int, default=600,
                       help='Population size for IPOP-CMA-ES (default: 600)')

    # Optuna study parameters
    parser.add_argument('--study_name', type=str, default='ipop_tuning',
                       help='Name for Optuna study (default: ipop_tuning)')
    parser.add_argument('--storage', type=str, default='sqlite:///optuna_ipop.db',
                       help='Optuna storage backend (default: sqlite:///optuna_ipop.db)')
    ...
    logger.info("=" * 80)
    logger.info("OPTUNA HYPERPARAMETER TUNING FOR IPOP-CMA-ES")
    logger.info("=" * 80)
    ...
    best_params_file = os.path.join(output_dir, 'best_ipop_params.json')
    best_params_data = {
