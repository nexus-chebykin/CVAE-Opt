# ------------------------------------------------------------------------------+
# Optuna-based Hyperparameter Tuning for Standard DE Optimizer
#
# This script performs automated hyperparameter search for the Standard DE
...
def setup_logging(output_dir: str) -> Tuple[str, logging.Logger]:
    ...
    # Create logger
    logger = logging.getLogger('optuna_de_tuning')
...
def create_config_for_trial(base_config, mutate: float, recombination: float):
    """
    Create a configuration object for a trial with specific DE parameters.
    ...
    Returns:
        Config object with DE parameters set
    """
    ...
    # Set DE-specific parameters (these are what we're tuning)
    config.de_mutate = mutate
    config.de_recombine = recombination
...
def objective(trial: optuna.Trial, args) -> float:
    ...
    logger.info(f"  Parameters: mutate={mutate:.6f}, recombination={recombination:.6f}")

    # Create config for this trial
    config = create_config_for_trial(base_config, mutate, recombination)
...
def main():
    parser = argparse.ArgumentParser(
        description="Optuna-based hyperparameter tuning for Standard DE optimizer"
    )
    ...
    parser.add_argument('--study_name', type=str, default='de_tuning',
                       help='Name for Optuna study (default: de_tuning)')
    parser.add_argument('--storage', type=str, default='sqlite:///optuna_de.db',
                       help='Optuna storage backend (default: sqlite:///optuna_de.db)')
    ...
    logger.info("=" * 80)
    logger.info("OPTUNA HYPERPARAMETER TUNING FOR STANDARD DE")
    logger.info("=" * 80)
    ...
    best_params_file = os.path.join(output_dir, 'best_de_params.json')
    best_params_data = {
