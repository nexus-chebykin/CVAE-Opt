# ------------------------------------------------------------------------------+
# EvoX CoDE (Composite Differential Evolution)
# Implementation using the EvoX optimization library
#
# Reference: https://evox.readthedocs.io/en/latest/apidocs/evox/evox.algorithms.so.de_variants.code.html
# CoDE: Wang, Y., Cai, Z., & Zhang, Q. (2011). Differential evolution with composite
# trial vector generation strategies and control parameters. IEEE transactions on
# evolutionary computation, 15(1), 55-66.
# ------------------------------------------------------------------------------+

import numpy as np
import time
import torch
from evox.algorithms.so.de_variants import CoDE
...
def minimize(cost_func, args, search_space_bound, search_space_size, popsize,
             ...
             diff_padding_num=5, replace=False, seed=1234):
    """
    Minimize objective function using EvoX CoDE algorithm.

    CoDE (Composite DE) combines three DE strategies (rand/1/bin, rand/2/bin, current-to-rand/1)
    with three parameter settings, using a pool-based parameter adaptation strategy.
    ...
    Args:
        ...
        mutate: Mutation factor (not used by CoDE, which uses three predefined parameter settings)
        recombination: Crossover rate (not used by CoDE, which uses three predefined parameter settings)
        ...
        diff_padding_num: Number of differential padding vectors for diversity (default: 5)
        replace: Population replacement strategy (default: False)
    ...
    # Initialize CoDE algorithm
    # CoDE uses a composite trial vector generation strategy with three DE strategies
    # and three parameter settings (F, CR) combinations
    # Reference: https://evox.readthedocs.io/en/latest/apidocs/evox/evox.algorithms.so.de_variants.code.html
