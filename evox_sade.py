# ------------------------------------------------------------------------------+
# EvoX SaDE (Self-adaptive Differential Evolution)
# Implementation using the EvoX optimization library
#
# Reference: https://evox.readthedocs.io/en/latest/apidocs/evox/evox.algorithms.so.de_variants.sade.html
# SaDE: Qin, A. K., & Suganthan, P. N. (2005). Self-adaptive differential evolution
# algorithm for numerical optimization. In 2005 IEEE congress on evolutionary
# computation (Vol. 2, pp. 1785-1791). IEEE.
# ------------------------------------------------------------------------------+
...
def minimize(cost_func, args, search_space_bound, search_space_size, popsize,
             ...
             diff_padding_num=9, LP=50, seed=1234):
    """
    Minimize objective function using EvoX SaDE algorithm.

    SaDE (Self-adaptive DE) adapts both F and CR parameters during evolution
    using a probability-based strategy selection mechanism. The algorithm
    maintains a pool of parameter values and strategies, selecting them based
    on their past success rates.
    ...
    Args:
        ...
        mutate: Mutation factor (not used by SaDE, which adapts F internally)
        recombination: Crossover rate (not used by SaDE, which adapts CR internally)
        ...
        LP: Learning period (memory size) for adaptation (default: 50)
    ...
    # Initialize SaDE algorithm
    # SaDE automatically adapts F and CR using probability-based strategy selection
    # Reference: https://evox.readthedocs.io/en/latest/apidocs/evox/evox.algorithms.so.de_variants.sade.html
