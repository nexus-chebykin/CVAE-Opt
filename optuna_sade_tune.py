...
def setup_logging(output_dir: str) -> Tuple[str, logging.Logger]:
    ...
    # Create logger
    logger = logging.getLogger('optuna_sade_tuning')
...
def create_config_for_trial(base_config, diff_padding_num: int, LP: int):
    """
    Create a configuration object for a trial with specific SaDE parameters.

    Args:
        base_config: Base configuration with model path, device, etc.
        diff_padding_num: Number of difference vectors
        LP: Learning period

    Returns:
        Config object with SaDE parameters set
    """
    ...
    # Set SaDE-specific parameters (these are what we're tuning)
...
def objective(trial: optuna.Trial, args) -> float:
    ...
    logger.info(f"  Parameters: diff_padding_num={diff_padding_num}, LP={LP}")

    # Create config for this trial
    config = create_config_for_trial(base_config, diff_padding_num, LP)
...
def main():
    parser = argparse.ArgumentParser(
        description="Optuna-based hyperparameter tuning for EvoX SaDE optimizer"
    )
    ...
    parser.add_argument('--batch_size', type=int, default=600,
                       help='Population size for SaDE (default: 600)')

    # Optuna study parameters
    parser.add_argument('--study_name', type=str, default='sade_tuning',
                       help='Name for Optuna study (default: sade_tuning)')
    parser.add_argument('--storage', type=str, default='sqlite:///optuna_sade.db',
                       help='Optuna storage backend (default: sqlite:///optuna_sade.db)')
    ...
    logger.info("=" * 80)
    logger.info("OPTUNA HYPERPARAMETER TUNING FOR EVOX SADE")
    logger.info("=" * 80)
    ...
    logger.info(f"  diff_padding_num: UniformInt[5, 15]")
    logger.info(f"  LP: UniformInt[30, 100]")
    logger.info("")
    logger.info("=" * 80)
    logger.info("")
    ...
    logger.info(f"  LP: {study.best_params['LP']}")
    logger.info(f"  Best mean gap: {study.best_value:.4f}%")
    logger.info("")

    # Save best parameters to JSON
    best_params_file = os.path.join(output_dir, 'best_sade_params.json')
    best_params_data = {
