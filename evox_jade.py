# ------------------------------------------------------------------------------+
# EvoX JADE (Adaptive Differential Evolution with Optional External Archive)
...
# JADE: Zhang, J., & Sanderson, A. C. (2009). JADE: Adaptive differential evolution
# with optional external archive. IEEE Transactions on evolutionary computation, 13(5), 945-958.
# ------------------------------------------------------------------------------+

import numpy as np
import time
import torch
from evox.algorithms.so.de_variants import JaDE
...
def minimize(cost_func, args, search_space_bound, search_space_size, popsize,
    ...
    Args:
        ...
        mutate: Mutation factor (not used by JADE, which adapts F internally)
        recombination: Crossover rate (not used by JADE, which adapts CR internally)
