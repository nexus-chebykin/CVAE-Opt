#!/usr/bin/env python3
# ------------------------------------------------------------------------------+
# Optuna-based Hyperparameter Tuning for Standard CMA-ES Optimizer
#
# This script performs automated hyperparameter search for the CMA-ES algorithm
...
#
# Outputs:
#   - best_cmaes_params.json: Best hyperparameters found
#   - optuna_cmaes_tuning.db: SQLite database with study results
...
def setup_logging(log_dir="logs"):
    ...
    log_file = Path(log_dir) / f"optuna_cmaes_tuning_{timestamp}.log"
...
def solve_instance(model, instance, sigma0, CMA_rankmu, CMA_rankone, cost_fn,
    ...
    def cost_func(Z_batch):
        """Vectorized cost function for CMA-ES optimizer."""
        with torch.no_grad():
    ...
    best_cost, best_solution, _, elapsed_time = cmaes.minimize(
        cost_func=cost_func,
        args=(),
...
def objective(trial: optuna.Trial, args) -> float:
    """
    Optuna objective function for CMA-ES hyperparameter tuning.
...
def save_results(study, args, log_file):
    ...
    output_file = "best_cmaes_params.json"
...
def generate_visualizations(study):
    ...
    try:
        ...
        fig.write_image("cmaes_optimization_history.png")
        logging.info("  Saved: cmaes_optimization_history.png")

        # Parameter importances (requires ≥10 trials)
        if len(study.trials) >= 10:
            fig = plot_param_importances(study)
            fig.write_image("cmaes_param_importances.png")
            logging.info("  Saved: cmaes_param_importances.png")

        # Parallel coordinate plot
        fig = plot_parallel_coordinate(study)
        fig.write_image("cmaes_parallel_coordinate.png")
        logging.info("  Saved: cmaes_parallel_coordinate.png")
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter tuning for CMA-ES optimizer using Optuna"
    )
    ...
    parser.add_argument('--storage', type=str, default='sqlite:///optuna_cmaes_tuning.db',
                       help='Optuna storage database (default: sqlite:///optuna_cmaes_tuning.db)')
    ...
    logging.info("=" * 80)
    logging.info("OPTUNA HYPERPARAMETER TUNING FOR CMA-ES OPTIMIZER")
    logging.info("=" * 80)
