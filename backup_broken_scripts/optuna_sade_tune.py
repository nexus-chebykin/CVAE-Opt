#!/usr/bin/env python3
# ------------------------------------------------------------------------------+
# Optuna-based Hyperparameter Tuning for EvoX SaDE Optimizer
#
# This script performs automated hyperparameter search for the SaDE algorithm
...
#
# Tunable Parameters:
#   - diff_padding_num: Number of differential padding vectors (range: [5, 15])
#   - LP: Learning period (memory size) for adaptation (range: [30, 100])
#
# Outputs:
#   - best_sade_params.json: Best hyperparameters found
#   - optuna_sade_tuning.db: SQLite database with study results
...
def setup_logging(log_dir="logs"):
    ...
    log_file = Path(log_dir) / f"optuna_sade_tuning_{timestamp}.log"
...
def solve_instance(model, instance, diff_padding_num, LP, cost_fn, batch_size,
                   time_limit, device):
    """
    ...
    def cost_func(Z_batch):
        """Vectorized cost function for SaDE optimizer."""
        with torch.no_grad():
    ...
    # Run SaDE optimizer
    best_cost, best_solution, _, elapsed_time = evox_sade.minimize(
        cost_func=cost_func,
        args=(),
        search_space_bound=search_space_bound,
        search_space_size=search_space_size,
        popsize=batch_size,
        mutate=None,  # Not used by SaDE
        recombination=None,  # Not used by SaDE
        maxiter=None,
        maxtime=time_limit,
        maxevaluations=None,
...
def objective(trial: optuna.Trial, args) -> float:
    """
    Optuna objective function for SaDE hyperparameter tuning.
    ...
    diff_padding_num = trial.suggest_int('diff_padding_num', 5, 15)
    LP = trial.suggest_int('LP', 30, 100)

    logging.info(f"Trial {trial.number} started")
    logging.info(f"  Parameters: diff_padding_num={diff_padding_num}, LP={LP}")
...
def save_results(study, args, log_file):
    ...
    output_file = "best_sade_params.json"
...
def generate_visualizations(study):
    ...
    try:
        ...
        fig.write_image("sade_optimization_history.png")
        logging.info("  Saved: sade_optimization_history.png")

        # Parameter importances (requires ≥10 trials)
        if len(study.trials) >= 10:
            fig = plot_param_importances(study)
            fig.write_image("sade_param_importances.png")
            logging.info("  Saved: sade_param_importances.png")

        # Parallel coordinate plot
        fig = plot_parallel_coordinate(study)
        fig.write_image("sade_parallel_coordinate.png")
        logging.info("  Saved: sade_parallel_coordinate.png")
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter tuning for SaDE optimizer using Optuna"
    )
    ...
    parser.add_argument('--storage', type=str, default='sqlite:///optuna_sade_tuning.db',
                       help='Optuna storage database (default: sqlite:///optuna_sade_tuning.db)')
    ...
    logging.info("=" * 80)
    logging.info("OPTUNA HYPERPARAMETER TUNING FOR SADE OPTIMIZER")
    logging.info("=" * 80)
