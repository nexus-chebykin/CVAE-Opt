#!/usr/bin/env python3
# ------------------------------------------------------------------------------+
# Optuna-based Hyperparameter Tuning for IPOP-CMA-ES Optimizer
#
# This script performs automated hyperparameter search for the IPOP-CMA-ES
...
#
# Outputs:
#   - best_ipop_params.json: Best hyperparameters found
#   - optuna_ipop_tuning.db: SQLite database with study results
...
def setup_logging(log_dir="logs"):
    ...
    log_file = Path(log_dir) / f"optuna_ipop_tuning_{timestamp}.log"
...
def solve_instance(model, instance, sigma0, CMA_rankmu, CMA_rankone, cost_fn,
    ...
    def cost_func(Z_batch):
        """Vectorized cost function for IPOP-CMA-ES optimizer."""
        with torch.no_grad():
            solutions = model.decode(Z_batch)
            costs = cost_fn(solutions, coordinates)
        return costs.cpu().numpy()

    # Run IPOP-CMA-ES optimizer
    best_cost, best_solution, _, elapsed_time = ipop_cmaes.minimize(
        cost_func=cost_func,
        args=(),
...
def objective(trial: optuna.Trial, args) -> float:
    """
    Optuna objective function for IPOP-CMA-ES hyperparameter tuning.
...
def save_results(study, args, log_file):
    ...
    output_file = "best_ipop_params.json"
...
def generate_visualizations(study):
    ...
    try:
        ...
        fig.write_image("ipop_optimization_history.png")
        logging.info("  Saved: ipop_optimization_history.png")

        # Parameter importances (requires ≥10 trials)
        if len(study.trials) >= 10:
            fig = plot_param_importances(study)
            fig.write_image("ipop_param_importances.png")
            logging.info("  Saved: ipop_param_importances.png")

        # Parallel coordinate plot
        fig = plot_parallel_coordinate(study)
        fig.write_image("ipop_parallel_coordinate.png")
        logging.info("  Saved: ipop_parallel_coordinate.png")
...
def main():
    parser = argparse.ArgumentParser(
        description="Hyperparameter tuning for IPOP-CMA-ES optimizer using Optuna"
    )
    ...
    parser.add_argument('--storage', type=str, default='sqlite:///optuna_ipop_tuning.db',
                       help='Optuna storage database (default: sqlite:///optuna_ipop_tuning.db)')
    ...
    logging.info("=" * 80)
    logging.info("OPTUNA HYPERPARAMETER TUNING FOR IPOP-CMA-ES OPTIMIZER")
    logging.info("=" * 80)
