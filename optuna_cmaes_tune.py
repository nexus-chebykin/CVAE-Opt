...
def setup_logging(output_dir: str) -> Tuple[str, logging.Logger]:
    ...
    # Create logger
    logger = logging.getLogger('optuna_cmaes_tuning')
...
def create_config_for_trial(base_config, sigma0: float, CMA_rankmu: float, CMA_rankone: float):
    """
    Create a configuration object for a trial with specific CMA-ES parameters.
    ...
    Returns:
        Config object with CMA-ES parameters set
    """
    ...
    # Set CMA-ES-specific parameters (these are what we're tuning)
    config.cmaes_sigma0 = sigma0
    config.cmaes_rankmu = CMA_rankmu
    config.cmaes_rankone = CMA_rankone
...
def main():
    parser = argparse.ArgumentParser(
        description="Optuna-based hyperparameter tuning for Standard CMA-ES optimizer"
    )
    ...
    parser.add_argument('--batch_size', type=int, default=600,
                       help='Population size for CMA-ES (default: 600)')

    # Optuna study parameters
    parser.add_argument('--study_name', type=str, default='cmaes_tuning',
                       help='Name for Optuna study (default: cmaes_tuning)')
    parser.add_argument('--storage', type=str, default='sqlite:///optuna_cmaes.db',
                       help='Optuna storage backend (default: sqlite:///optuna_cmaes.db)')
    ...
    logger.info("=" * 80)
    logger.info("OPTUNA HYPERPARAMETER TUNING FOR STANDARD CMA-ES")
    logger.info("=" * 80)
    ...
    best_params_file = os.path.join(output_dir, 'best_cmaes_params.json')
    best_params_data = {
