...
class TransformerBasedDecoder(nn.Module):
    ...
    def __init__(self, instance_embedding, search_space_size, mask_fn, update_fn):
        ...
        for i in range(num_layers):
            layer = nn.ModuleDict({
                'mha': nn.MultiheadAttention(embed_dim=model_size, num_heads=num_heads, dropout=0, batch_first=True),
                'mlp': nn.Sequential(
                    nn.Linear(model_size, expand_factor * model_size),
                    nn.GELU(),
                    nn.Linear(expand_factor * model_size, model_size),
        ...
        self.pred_embedding = nn.Linear(model_size, expand_factor * model_size),
        ...
        # NO cost_predictor for transformer-only model (no regression head)
    ...
    def forward(self, instance, solution, Z, instance_hidden, config, teacher_forcing, pred_cost = True):
        ...
        # Transformer-only model: no cost prediction
        p_cost = None
